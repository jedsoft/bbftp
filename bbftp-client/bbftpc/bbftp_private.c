    ...
    if ( (msg_sec->crtype & CRYPT_RSA_PKCS1_OAEP_PADDING ) == CRYPT_RSA_PKCS1_OAEP_PADDING) {
        ...
        */
        ...
        if ( (hisrsa = RSA_new()) == NULL) {
            free(readbuffer) ;
            close(prv_ctrlsock) ;
            printmessage(stderr,CASE_ERROR,56, "Error reading encrypted message : %s (%s)\n","getting RSA",(char *) ERR_error_string(ERR_get_error(),NULL)) ;
            return -1 ;
        }
        /*
        ...
        */
        ...
        if (BBftp_Debug) printmessage(stdout,CASE_NORMAL,0, "Public daemon RSA key received\n") ;
        /*
        ...
        */
        if ( (myrsa = RSA_generate_key(NBITSINKEY,3,NULL,NULL)) == NULL) {
            free(readbuffer) ;
            close(prv_ctrlsock) ;
            printmessage(stderr,CASE_ERROR,27, "Error while private authentication : cannot generate keys : %s\n",ERR_error_string(ERR_get_error(),NULL) ) ;
            return -1 ;
        }
        /*
        ...
        */
        ...
        if (BBftp_Debug) printmessage(stdout,CASE_NORMAL,0, "Public client RSA key sent\n") ;
        free(readbuffer) ;
        /*
        ...
        */
        if ( bbftp_private_auth(logmessage) < 0 ) {
            close(prv_ctrlsock) ;
            printmessage(stderr,CASE_ERROR,27, "Error while private authentication : %s\n",logmessage) ;
            return -1 ;
        } else {
            /*
        ...
        free(readbuffer) ;
        close(prv_ctrlsock) ;
        printmessage(stderr,CASE_ERROR,57, "Unkown encryption method \n") ;
        return -1  ;
    }
}
...
int bbftp_private_send(char *buffertosend, int buffertosendlength, char *logmessage)
{
    ...
    */
    ...
    if (BBftp_Debug) printmessage(stdout,CASE_NORMAL,0, "Going to send %d Bytes\n",buffertosendlength) ;
    ...
    */
    ...
    if (BBftp_Debug) printmessage(stdout,CASE_NORMAL,0, "Number of encrypted packet(s) : %d\n",nbpackets) ;
    /*
    ...
    */
    ...
    for (i=1; i<= nbpackets ; i++ ) {
        ...
        if (BBftp_Debug) printmessage(stdout,CASE_NORMAL,0, "Crypting %d Bytes\n",lentocrypt) ;
    ...
    if (BBftp_Debug) printmessage(stdout,CASE_NORMAL,0, "%d encrypted packet(s) sent\n",nbpackets) ;
    return 0 ;
}
...
int bbftp_private_recv(char *buffertorecv, int lengthtorecv, char *logmessage)
{
    ...
    */
    ...
    if (BBftp_Debug) printmessage(stdout,CASE_NORMAL,0, "Received %d Bytes\n",totallendecrypted) ;
