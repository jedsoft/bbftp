...
int bbftp_stat(char *filename,int  *errcode) 
{
    ...
    if ( verbose) printmessage(stdout,CASE_NORMAL,0,timestamp,">> COMMAND : stat %s\n",filename) ;
    ...
    ** Directory name
    */
    if ( writemessage(outcontrolsock,filename,strlen(filename),sendcontrolto,0) < 0 ) {
        printmessage(stderr,CASE_ERROR,64,timestamp,"Error sending %s message\n","MSG_STAT (file name)");
        *errcode = 64 ;
        bbftp_close_control() ;
        return BB_RET_CONN_BROKEN ; /* restart connection */
        ...
        */
        ...
        if ( code == MSG_BAD || code == MSG_BAD_NO_RETRY) {
            ...
            */
            ...
            if ( statoutput ) {
                printmessage(stdout,CASE_NORMAL,0,0,"stat %s\n", buffer) ;
            }
            free(buffer) ;
            return BB_RET_OK ;
        } else {
