...
int bbftp_list(char *line,char **filelist,int *filelistlen,int *errcode)
{
    ...
    if ( BBftp_Verbose) printmessage(stdout,CASE_NORMAL,0, ">> COMMAND : list %s\n",line) ;
    ...
    ** Directory name
    */
    if ( writemessage(BBftp_Outcontrolsock,line,strlen(line),BBftp_Sendcontrolto) < 0 ) {
        ...
        if ( code == MSG_BAD || code == MSG_BAD_NO_RETRY) {
            ...
            */
            ...
            if ( msglen == 0 ) {
                ...
                */
                if ( BBftp_Verbose) printmessage(stdout,CASE_NORMAL,0, "<< OK : no file\n") ;
                *filelistlen = 0 ; 
                return BB_RET_OK ;
            } else {
                ...
                if ( readmessage(BBftp_Incontrolsock,*filelist,msglen,BBftp_Recvcontrolto) < 0) {
