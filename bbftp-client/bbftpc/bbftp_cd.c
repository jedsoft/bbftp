...
/****************************************************************************
 ...
 bbftp_cd.c v 2.0.0 2001/03/13  - Creation of the routine 
...
int bbftp_cd(char *dirpath,int  *errcode) 
{
    ...
    if ( verbose) printmessage(stdout,CASE_NORMAL,0,timestamp,">> COMMAND : cd %s\n",dirpath) ;
    
    msg = (struct message *)minbuffer ;
    msg->code = MSG_CHDIR_V2 ;
    ...
    ** Directory name
    */
    if ( writemessage(outcontrolsock,dirpath,strlen(dirpath),sendcontrolto,0) < 0 ) {
        ...
        */
        ...
        if ( code == MSG_BAD || code == MSG_BAD_NO_RETRY) {
            ...
            */
            ...
            if ( verbose) printmessage(stdout,CASE_NORMAL,0,timestamp,"<< OK : Current remote directory is %s\n",remotedir) ;
            return BB_RET_OK ;
        } else {
            /*
