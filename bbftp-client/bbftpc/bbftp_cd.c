...
int bbftp_cd(char *dirpath,int  *errcode) 
{
    ...
    if ( BBftp_Verbose) printmessage(stdout,CASE_NORMAL,0,BBftp_Timestamp,">> COMMAND : cd %s\n",dirpath) ;
            ...
            if ( readmessage(BBftp_Incontrolsock,buffer,msglen,BBftp_Recvcontrolto,0) < 0) {
                ...
                *errcode = 67 ;
                ...
                if ( BBftp_Remotedir != NULL ) {
                    free(BBftp_Remotedir) ;
                }
                BBftp_Remotedir = buffer ;
            }
            if ( BBftp_Verbose) printmessage(stdout,CASE_NORMAL,0,BBftp_Timestamp,"<< OK : Current remote directory is %s\n",BBftp_Remotedir) ;
            return BB_RET_OK ;
        } else {
            /*
