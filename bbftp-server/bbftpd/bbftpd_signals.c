...
*/
 ...
 #define nz(x)   ((x) == 0 ? 1 : (x))
                ...
                bbftpd_log(BBFTPD_NOTICE, "%s", logmessage);
            }
            free_all_var() ;
        } else {
            state = S_LOGGED ;
...
int bbftpd_blockallsignals() {
    ...
    if ( sigaction(SIGABRT,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGABRT : %s",strerror(errno)) ;
        return(-1) ;
    }
    if ( sigaction(SIGALRM,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGALRM : %s",strerror(errno)) ;
        return(-1) ;
    }
    if ( sigaction(SIGHUP,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGHUP : %s",strerror(errno)) ;
    ...
    if ( sigaction(SIGINT,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGINT : %s",strerror(errno)) ;
        return(-1) ;
    }
    if ( sigaction(SIGPIPE,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGPIPE : %s",strerror(errno)) ;
    ...
    if ( sigaction(SIGQUIT,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGQUIT : %s",strerror(errno)) ;
        return(-1) ;
    }
/*
**    if ( sigaction(SIGTERM,&sga,0) < 0 ) {
...
*/
    if ( sigaction(SIGUSR1,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGUSR1 : %s",strerror(errno)) ;
        return(-1) ;
    }
    if ( sigaction(SIGUSR2,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGUSR2 : %s",strerror(errno)) ;
        return(-1) ;
    }
    if ( sigaction(SIGCHLD,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGCHLD : %s",strerror(errno)) ;
    ...
    if ( sigaction(SIGTSTP,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGTSTP : %s",strerror(errno)) ;
        return(-1) ;
    }
#ifndef DARWIN
    if ( sigaction(SIGPOLL,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGPOLL : %s",strerror(errno)) ;
        return(-1) ;
    }
#endif
#ifdef SIGPROF		/* BUG porting */
    if ( sigaction(SIGPROF,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGPROF : %s",strerror(errno)) ;
        return(-1) ;
    }
#endif
    if ( sigaction(SIGURG,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGURG : %s",strerror(errno)) ;
        return(-1) ;
    }
#ifdef SIGVTALRM	/* BUG porting */
    if ( sigaction(SIGVTALRM,&sga,0) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error sigaction SIGVTALRM : %s",strerror(errno)) ;
        return(-1) ;
    }
#endif
    return 0 ;
