...
/* #endif */
    if ( msglen > (int)MAXMESSLEN ) {
        ...
        ** big
        */
        bbftpd_log(BBFTPD_ERR,"Message to big in changetodir (%d,%lu)",msglen,MAXMESSLEN) ;
    ...
    */
    ...
    if ( code == MSG_CHDIR ) {
        bbftpd_log(BBFTPD_DEBUG,"Changing directory to %s",receive_buffer) ;
    }
    /*
    ...
    */
    if ( chdir(receive_buffer) < 0 ) {
        ...
        */
        ...
        bbftpd_log(BBFTPD_ERR,"Error changing directory %s : %s",receive_buffer,strerror(errno)) ;
        /*
