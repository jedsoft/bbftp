...
int sendafilerfio(int code) {
    ...
    */ 
    if ( (retcode = readmessage(msgsock,receive_buffer,STORMESSLEN,recvcontrolto) ) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"Error receiving file char") ;
        return retcode ;
    }
    ...
    if ( code == MSG_RETR_RFIO ) {
        ...
        bbftpd_log(BBFTPD_DEBUG,"Retreiving rfio file %s with %d children",readfilename,msg_store->nbport) ;
    } else {
        compressiontype = COMPRESSION ;
        bbftpd_log(BBFTPD_DEBUG,"Retreiving rfio file %s with %d children in compressed mode",readfilename,msg_store->nbport) ;
    }
    /*
    ** WARNING ----------
    **
    ...
    */
    if ( rfio_stat(readfilename,&statbuf) < 0 ) {
        ...
        */
        ...
        bbftpd_log(BBFTPD_ERR,"Error stating file %s : %s ",readfilename,strerror(savederrno)) ;
        ...
        */
        if ( (statbuf.st_mode & S_IFDIR) == S_IFDIR) {
            bbftpd_log(BBFTPD_ERR,"file %s is a directory",readfilename) ;
    ...
    */
    if ( (fd = open(readfilename,O_RDONLY)) < 0 ) {
        ...
        */
        savederrno = errno ;
        bbftpd_log(BBFTPD_ERR,"Error opening local file %s : %s",readfilename,strerror(errno)) ;
    ...
    if ( (filelen = lseek(fd,0,SEEK_END) ) < 0 ) {
        ...
        */
        close(fd) ;
        bbftpd_log(BBFTPD_ERR,"Error seeking local file %s : %s",readfilename,strerror(errno)) ;
    ...
    */
    ...
    if ( writemessage(msgsock,send_buffer,MINMESSLEN,recvcontrolto) < 0 ) {
        ...
        */
        bbftpd_log(BBFTPD_ERR,"Error sending RETROK part 1") ;
        return -1 ;
    }
    ...
    if ( writemessage(msgsock,send_buffer,RETROKMESSLEN,recvcontrolto) < 0 ) {
        ...
        ** tell calling to close the connection
        */
        bbftpd_log(BBFTPD_ERR,"Error sending RETROK part 2") ;
        return -1 ;
    }
    /*
    ...
    */
    if ( readmessage(msgsock,receive_buffer_sup,MINMESSLEN,RETRSTARTTO) < 0 ) {
        ...
        ** tell calling to close the connection
        */
        bbftpd_log(BBFTPD_ERR,"Error receiving RETRSTART") ;
        return -1 ;
    }
    ...
    if ( msg->code == MSG_ABR) {
        ...
        */
        ...
        bbftpd_log(BBFTPD_INFO,"Send zero length file") ;
        return 0 ;
    } else if ( msg->code != MSG_RETR_START ) {
        bbftpd_log(BBFTPD_ERR,"Receive Unknown message code while waiting RETRSTART %d",msg->code) ;
        return -1 ;
    }
    /*
