...
int sendafilerfio(int code) {
    ...
    */ 
    ...
    if ( code == MSG_RETR_RFIO ) {
        ...
        bbftpd_syslog(BBFTPD_DEBUG,"Retreiving rfio file %s with %d children",readfilename,msg_store->nbport) ;
    } else {
        compressiontype = COMPRESSION ;
        bbftpd_syslog(BBFTPD_DEBUG,"Retreiving rfio file %s with %d children in compressed mode",readfilename,msg_store->nbport) ;
    }
    /*
    ** WARNING ----------
    **
