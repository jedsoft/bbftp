...
int bbftpd_private_receive_connection(int msglen) 
{
    ...
    */
    if ( (receive_buffer = (char *) malloc (msglen+1)) == NULL ) {
        bbftpd_log(BBFTPD_ERR,"Error allocation memory for receive_buffer (bbftpd_private_receive_connection) ") ;
    ...
    if ( msg_sec->crtype ==  CRYPT_RSA_PKCS1_OAEP_PADDING) {
        ...
        if ( (hisrsa = RSA_new()) == NULL) {
            FREE(receive_buffer) ;
            bbftpd_log(BBFTPD_ERR,"Error RSA_new (bbftpd_private_receive_connection) ") ;
        ...
        */
        if ( (hisrsa->n = BN_new()) == NULL) {
            FREE(receive_buffer) ;
            bbftpd_log(BBFTPD_ERR,"Error BN_new (bbftpd_private_receive_connection) ") ;
            strcat(logmessage," : RSA Error") ;
            reply(MSG_BAD,logmessage) ;
            return -1 ;
        }
        ...
        */
        if ( BN_mpi2bn(pubkey,lenkey,hisrsa->n) == NULL ) {
            FREE(receive_buffer) ;
            bbftpd_log(BBFTPD_ERR,"Error BN_mpi2bn (bbftpd_private_receive_connection) ") ;
            strcat(logmessage," : RSA Error") ;
            reply(MSG_BAD,logmessage) ;
            return -1 ;
        }
    ...
    */
    if ( bbftpd_private_auth(logmessage) < 0 ) {
        bbftpd_log(BBFTPD_ERR,"bbftpd_private_auth failed : %s",logmessage) ;
        reply(MSG_BAD,logmessage) ;
        return -1 ;
    }
    return 0 ;
